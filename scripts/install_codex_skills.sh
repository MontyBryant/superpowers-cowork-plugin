...
usage() {
  cat <<'EOF'
Install superpowers-knowledge skills globally for Codex.
...
Modes:
  --from-github    Clone or update the repo at $CODEX_HOME/vendor_imports/repos/superpowers-knowledge-plugin
                   and install skills from there.

  default          Install from the current checkout (or --source-root).

Options:
  --force          Replace existing superpowers-knowledge Codex skill links if present.
...
FORCE=0
REF="main"
REPO_URL="https://github.com/montymerlin/superpowers-knowledge-plugin.git"
...
if [[ "$FROM_GITHUB" -eq 1 ]]; then
  SOURCE_ROOT="$CODEX_HOME/vendor_imports/repos/superpowers-knowledge-plugin"
...
for skill_dir in \
  ...
  writing-plans \
  writing-skills; do
  install_link "superpowers-knowledge-${skill_dir}" "$SOURCE_ROOT/skills/${skill_dir}"
done

echo "Installed superpowers-knowledge skills into $CODEX_HOME/skills"
